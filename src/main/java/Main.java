
public class Main {
	/**
	 * Fucking awesome main function
	 * @param args
	 */
	public static void main(String[] args) {
		System.out.println("1 2 3 próba system outa <- ktos to napisal na windowsie? jezeli tak to chyba trzeba ustawic kodowanie utf8");
		System.out.println("Zomfbiaki");
		System.out.println("Roger that@Karol");
		System.out.println("Pull z laptopa");
		System.out.println("a tera ze stogu siana w stodole");
		System.out.println("patataj");
		System.out.println("no witam");
		System.out.println("pykam jeszcze raz, dodalem doskonalego javadoca"); //anything
	}

}
