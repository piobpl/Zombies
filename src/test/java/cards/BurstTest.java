...
public class BurstTest {
	...
	public void test() {
		...
		new Zombie(2).makeEffect(ForTestsOnly.getNewCellSelection(3, 1), controller.gameState);
		new Zombie(3).makeEffect(ForTestsOnly.getNewCellSelection(2, 1), controller.gameState);
		assertEquals(true, burst.isSelectionCorrect(controller.gameState, selection));
		...
		assertEquals(new Integer(1), controller.gameState.getBoard().get(1,1).getStrength());
	}
}
